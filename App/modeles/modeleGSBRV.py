...
def getRapportsVisite( matricule , mois , annee ) :
	try :
		...
		for unEnregistrement in enregistrements :
			...
			unRapport['pra_num'] = unEnregistrement[3]
...
def enregistrerRapportVisite( matricule , numPraticien , dateVisite , bilan, nummotif, coefconfiance) :
	...
	if numRapportVisite != None :
		...
		try:
			...
			requete = '''
				insert into RapportVisite( vis_matricule , rap_num , rap_date_visite , rap_bilan , pra_num, mo_nummotif, rap_coefconfiance )
				values( %s , %s , %s , %s , %s , %s, %s )
				'''

			curseur.execute( requete, ( matricule , numRapportVisite , dateVisite , bilan , numPraticien, nummotif, coefconfiance) )
			connexionBD.commit()
			curseur.close()
